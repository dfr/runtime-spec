...
* **`ip4`** *(string, OPTIONAL)* - control the availability of IPv4 addresses.
    The value can be "new" which allows the addresses listed in `ip4.addr` to be used, "inherit" which allows all addresses in the jail's vnet or "disable" to stop use of IPv4 entirely.
* **`ip6`** *(string, OPTIONAL)* - control the availability of IPv6 addresses.
    The value can be "new" which allows the addresses listed in `ip6.addr` to be used, "inherit" which allows all addresses in the jail's vnet or "disable" to stop use of IPv6 entirely.
